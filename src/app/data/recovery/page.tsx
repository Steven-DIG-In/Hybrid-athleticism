...
export default async function RecoveryPage() {
    ...
    // user is guaranteed non-null because getRecoveryAnalytics already succeeded.
    const series = user
        ? await getRecentCoachDeltaSeries(user.id, 'recovery', { limit: 20 })
        : []
    ...
    return (
        <div className="animate-in fade-in duration-500 flex flex-col gap-4">
            <PatternFlagCard flag={flag} coach="recovery" />
            <PerformanceDeltaChart title="recovery performance deltas" points={points} />
            <RecoveryDashboard data={result.data} />
        </div>
    )
}
