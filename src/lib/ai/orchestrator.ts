...
// Prompts — Pipeline B uses head-coach (adjustment directive) only.
// Per-domain prompt builders are read off coachRegistry at runtime via meta.
import {
...
// extractWeekBrief was relocated to `@/lib/engine/mesocycle/strategy.ts`.
...
// generateMesocycleProgram (Pipeline A) was relocated to
// `@/lib/engine/mesocycle/generate.ts` in Task 11 — merged with the
// previous coaching.actions.generateMesocycleWithCoaches wrapper into a
