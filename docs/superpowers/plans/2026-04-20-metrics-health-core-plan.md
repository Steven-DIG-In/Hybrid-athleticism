...
**Files:**
- Read: `supabase/migrations/015_training_maxes.sql`, `014_training_day_model_cleanup.sql`, `002_rls_policies.sql`
- Read: `src/lib/types/database.types.ts`

- [ ] **Step 1: Confirm next migration number is 016**
...
```
Expected: highest numbered is `015_training_maxes.sql` (ignore `20260224...` timestamp-style). If a newer numbered migration exists, renumber this plan's migration accordingly and update references below.

- [ ] **Step 2: Confirm existing RLS pattern**

Run:
```bash
grep -E "CREATE POLICY|auth.uid" supabase/migrations/002_rls_policies.sql | head -20
```
Expected: policies use `auth.uid() = user_id` pattern. This plan follows the same.

- [ ] **Step 3: Confirm Supabase Vault + pgsodium are available**

Run:
```bash
grep -r "pgsodium\|vault\.secrets" supabase/migrations/ 2>/dev/null
```
Expected: Vault is enabled in hosted Supabase by default; nothing to install. If your project hasn't enabled it, you'll see an error when Plan 3 tries to insert credentials. Not a blocker for this plan.

- [ ] **Step 4: No commit — inspection only**

---

## Task 2: Write migration 016 — tables, storage, RLS

**Files:**
- Create: `supabase/migrations/016_metrics_dashboard.sql`

- [ ] **Step 1: Write the migration file**

```sql
-- ============================================================================
-- Migration 016: Metrics Dashboard — Training Adherence + Health Extension
-- Spec: docs/superpowers/specs/2026-04-20-metrics-dashboard-health-extension-design.md
-- ============================================================================

-- Training adherence: agent_interventions extensions (verify-or-add)
-- If the table doesn't exist yet, create; otherwise add missing columns.
DO $$ BEGIN
  ...
  ELSE
    -- Verify columns; add only if missing
...
-- Training adherence: off_plan_sessions
...
-- Training adherence: performance_deltas.delta_magnitude_pct (generated)
DO $$ BEGIN
...
-- ============================================================================
-- Health: Garmin (credentials table lives here; population is Plan 3)
-- ============================================================================
...
- [ ] **Step 2: Run migration locally (Supabase CLI)**

Run:
```bash
cd "/Users/steven/Vibe Projects/hybrid-athleticism"
supabase db reset --no-seed  # or: supabase migration up
```
Expected: migration applies clean. If `performance_deltas` or `agent_interventions` don't exist yet in your local DB, the DO blocks skip safely.
...
```
Expected: `database.types.ts` updates; imports compile.

- [ ] **Step 4: Commit**

```bash
git add supabase/migrations/016_metrics_dashboard.sql src/lib/types/database.types.ts
git commit -m "feat(migration): 016 metrics dashboard health + adherence tables"
```
